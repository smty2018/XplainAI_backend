...
The following example specifies the output file name (with the -o flag), renders only the first ten animations ( -n flag) with a white
background ( -c flag), and saves the animation as a .gif instead of as a .mp4 file ( --format=gif flag).  It uses the default quality and does not try to
...
Global options:
  ...
  --seed INTEGER                 Set the random seed to allow reproducibility.
...
are interchangeable, but the config file only accepts background_color as an admissible option.
...
will use the config file found in path/to/file.py , if any.  It will not use the config file found in the current working directory, even if it exists.
...
using the previous two config files and executing manim -c RED <file.py> SceneName is equivalent to not using any config files and
executing
...
will override the library-wide file.  This is referred to as the cascading config file system.
...
1. The user-wide and folder-wide files are loaded if they exist.
1. All files found in the previous two steps are parsed in a single ConfigParser object, called parser .  This is where cascading happens.
1. logging.Logger is instantiated to create Manim’s global logger object. It is configured using the “logger” section of the parser,
i.e. parser['logger'] .
1. ManimConfig is instantiated to create the global config object.
1. The parser from step 3 is fed into the config from step 5 via ManimConfig.digest_parser() .
1. Both logger and config are exposed to the user.
...
1. If the --config_file flag was used, a new ConfigParser object
...
1. The new parser is fed into config .
1. The rest of the CLI flags are processed.
...
1. user-wide config file, if it exists,
1. folder-wide config file, if it exists OR custom config file, if passed via --config_file ,
1. other CLI flags, and
1. any programmatic changes made after the config system is set.
